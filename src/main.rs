...
fn main() {
    if let Err(e) = run_tool() {
        Logger::log_error(&format!("An error has occurred: {:?}", e));
        std::process::exit(1);
    }
}
...
fn perform_build(root_config: &SporaConfig) -> Result<()> {
    ...
    if let Some(ws) = &root_config.workspace {
        ws.members.par_iter().for_each(|member_name| {
            ...
            if config_path.exists() {
                let m_config = SporaConfig::load_from_path(&config_path)
                    .expect("Failed to load module config");
                if let Some(p) = &m_config.project {
                    build_module(member_path, p, root_config).expect("Failed to build module");
                }
            }
        });
    } else if let Some(p) = &root_config.project {
        build_module(Path::new("."), p, root_config).context("Failed to build module")?;
    }
    Ok(())
}

fn build_module(path: &Path, project: &crate::config::Project, root_config: &SporaConfig) -> Result<()> {
    Builder::bloom(
        path,
        &project.name,
        &project.lang,
...
fn create_simple_jar(path: &Path, project: &config::Project, root_config: &SporaConfig) -> Result<()> {
...
fn create_flat_jar(path: &Path, project: &config::Project, root_config: &SporaConfig) -> Result<()> {
    let jar_name = format!("{}.jar", project.name);
    ...
    fs::write(&manifest_path, manifest_content).expect("Failed to create manifest");
...
fn create_fat_jar(path: &Path, project: &config::Project, root_config: &SporaConfig, fs_lock: Arc<Mutex<()>>) -> Result<()> {
    let jar_name = format!("{}.jar", project.name);
    let temp_unpack = path.join("out_fat");
    
    let lock_unchanged = Builder::is_lock_unchanged(path).context("Failed to verify if the lock file matches the current configuration")?;
    ...
    if lock_unchanged && cache_exists {
        ...
        fs::create_dir_all(&temp_unpack).expect("Failed to create temp directory");
    ...
    Logger::log_step("Merging", "project classes...");
...
fn unpack_jar(jar_path: &Path, dest_dir: &Path, fs_lock: Arc<Mutex<()>>) {
    let file = fs::File::open(jar_path).expect("Failed to open JAR");
    let mut archive = ZipArchive::new(file).expect("Invalid JAR format");
...
fn run_tool() -> Result<()> {
    ...
    match command {
        "init" => {
            ...
            let default_config = r#"# Spora Project Configuration
...
# Set to true only if you agree to the Oracle Technology Network License Agreement.
# accept_oracle_licence_terms = false
...
"#;
    ...
    public static void main(String[] args) {
        ...
        "fetch" => {
            ...
            perform_fetch(&root_config, &resolver, &project_root).context("Failed to fetch")?;
        },
        "build" => {
            let root_config = SporaConfig::load()
                .context("Failed to load spora.toml. Please check that the file exists")?;
            ...
            perform_build(&root_config).context("Failed to build")?;
        },
        "bloom" => {
            let root_config = SporaConfig::load()?;
        ...
        "run" => {
            let config = SporaConfig::load().context("Failed to load spora.toml")?;
